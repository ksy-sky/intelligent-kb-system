...
class IntelligentAssistant:
    ...
    def _search_relevant_info(self, question: str) -> List[Dict]:
        ...
        stop_words = {
            "как", "дела", "привет", "пока", "что", "такое", "чем", "где", "когда", "почему",
            "расскажи", "покажи", "объясни", "опиши", "найди", "дай",
            "и", "или", "в", "на", "с", "по", "к", "от", "для", "про", "о",
            "существуют", "бывают", "есть", "можете", "назвать", "нужен", "нужна",
            "отличие", "различие", "разница", "сравнение", "сравни",
            "информации", "информация", "данных", "данные",
            "это", "не", "да", "нет", "бы", "же", "ли", "то", "всё", "все",
            "составные", "части", "состав", "компоненты", "символы", "относятся",
            "какие", "перечисли", "список", "используется", "применяется"
        }
    ...
    def _build_context(self, concepts: List[Dict]) -> str:
        ...
        for c in concepts:
            ...
            if c.get("examples"):
                examples = [str(e) for e in c['examples'] if not str(e).startswith("./")]
                if examples:
            ...
            if c.get("relations"):
                ...
                for rel_type, targets in c["relations"].items():
                    resolved = [resolve_id(t) for t in targets if not str(t).startswith("./")]
                    if resolved:
    ...
    def _ask_llm(self, question: str, concepts: List[Dict]) -> Dict[str, Any]:
        ...
        #  тип вопроса
        if any(w in question_lower for w in ["отличие", "различие", "разница", "сравни", "сравнение", "отличается"]):
...
Ответь в JSON: {{"success": true, "type": "comparison", "message": "твой ответ"}}"""
        ...
        elif any(w in question_lower for w in ["что такое", "определение", "дай понятие", "объясни"]):
            prompt = f"""Контекст из базы знаний ПиОИвИС:
...
Ответь в JSON: {{"success": true, "type": "definition", "message": "твой ответ"}}"""
        ...
        elif any(w in question_lower for w in ["какие", "перечисли", "список", "относится", "входят", "состав", "компоненты", "метасимвол", "символ"]):
            prompt = f"""Контекст из базы знаний ПиОИвИС:
...
Ответь в JSON: {{"success": true, "type": "classification", "message": "твой ответ с перечислением"}}"""
        ...
        elif any(w in question_lower for w in ["связи", "связан", "отношения", "используется", "применяется", "состоит"]):
            prompt = f"""Контекст из базы знаний ПиОИвИС:
...
Ответь в JSON: {{"success": true, "type": "general", "message": "твой ответ"}}"""
    ...
    def _fallback_search(self, question: str, concepts: List[Dict]) -> Dict[str, Any]:
        ...
        if not concepts:
            ...
            general_words = ["как дела", "привет", "здравствуй", "пока", "спасибо", "как жизнь", "кто ты"]
    ...
    def process_question(self, question: str) -> Dict[str, Any]:
        ...
        if not question or not question.strip():
            return {"success": False, "type": "error", "message": "Пожалуйста, задайте вопрос."}
        ...
        for word in general_words:
            if word in question_lower:
                return {"success": True, "type": "general", "message": "Здравствуйте! Я — интеллектуальный ассистент по дисциплине ПиОИвИС. Буду рад ответить на ваши вопросы по теме дисциплины!"}
        
        # Ищем термины
        concepts = self._search_relevant_info(question)
        
        # Типы вопросов, которые можно обработать локально
        need_definition = any(w in question_lower for w in ["что такое", "определение", "объясни", "кто такой", "что значит"])
        need_composition = any(w in question_lower for w in ["состоит", "состав", "компонент", "части", "из чего", "что входит", "что содержит"])
        need_usage = any(w in question_lower for w in ["используется", "применяется", "где", "для чего"])
        need_classification = any(w in question_lower for w in ["виды", "типы", "классификац", "разбиение", "какие бывают", "на что делится", "перечисли", "какие есть"])
        need_examples = any(w in question_lower for w in ["пример", "примеры", "приведи"])
        need_hierarchy = any(w in question_lower for w in ["надкласс", "родитель", "что шире", "что включает"])
        
        # Если есть термины и это простой вопрос — отвечаем локально
        if concepts and any([need_definition, need_composition, need_usage, need_classification, need_examples, need_hierarchy]):
            ...
            for c in all_concepts:
                if c.get("definition") and (not merged_definition or c.get("source") == "theory"):
                ...
                for k, v in c.get("relations", {}).items():
                    ...
                    for item in v:
                        ...
                        if not str(item).startswith("./") and resolved not in merged_relations[k]:
                            merged_relations[k].append(resolved)
            
            if need_definition and merged_definition:
                return {"success": True, "type": "definition", "message": f"{term_name} — это {merged_definition}"}
            
            if need_composition:
                for key in ["составные части", "состав", "компоненты", "элементы", "структура"]:
                    if key in merged_relations and merged_relations[key]:
                        return {"success": True, "type": "relations", "message": f"Состав «{term_name}»:\n" + "\n".join(f"• {item}" for item in merged_relations[key])}
            
            if need_usage:
                for key in ["используется в", "применяется в", "используется для"]:
                    if key in merged_relations and merged_relations[key]:
                        return {"success": True, "type": "relations", "message": f"«{term_name}» используется в:\n" + "\n".join(f"• {item}" for item in merged_relations[key])}
            
            if need_classification:
                for key in ["разбиение", "классификация", "виды"]:
                    if key in merged_relations and merged_relations[key]:
                        return {"success": True, "type": "classification", "message": f"Классификация «{term_name}»:\n" + "\n".join(f"• {item}" for item in merged_relations[key])}
            
            if need_examples and merged_examples:
                return {"success": True, "type": "examples", "message": f"Примеры «{term_name}»:\n" + "\n".join(f"• {e}" for e in merged_examples[:10])}
            
            if need_hierarchy:
                for key in ["надкласс", "надклассы"]:
                    if key in merged_relations and merged_relations[key]:
                        return {"success": True, "type": "hierarchy", "message": f"Надклассы «{term_name}»:\n" + "\n".join(f"• {item}" for item in merged_relations[key])}
        ...
        if concepts:
            ...
            for c in all_concepts:
                if c.get("definition"):
                    return {"success": True, "type": "definition", "message": f"{c['term']} — это {c['definition']}"}
        
        return {"success": False, "type": "error", "message": "Не удалось найти информацию. Попробуйте переформулировать вопрос."}

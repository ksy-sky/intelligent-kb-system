...
class IntelligentAssistant:
    ...
    - базы данных (реляционные, графовые)
    - регулярные выражения и формальные языки
    - форматы данных (JSON, XML, CSV)
    - кодирование информации
    - структуры данных и алгоритмы
    - языки запросов (SQL, SPARQL)
    - логическое программирование
    - интеллектуальные системы

    Вопрос: "{question}"

    Вопросы про блокчейн, криптовалюты, веб-разработку, железо, операционные системы, нейросети, общие вопросы про компьютеры — НЕ относятся к дисциплине.

    Если вопрос относится к темам выше — ответь СТРОГО "ДА".
    ...
    Не пиши ничего кроме "ДА" или "НЕТ"."""
        ...
        base_instruction = (
            "Ты — ассистент по дисциплине ПиОИвИС.\n"
            "Отвечай на вопросы, используя ТОЛЬКО информацию из контекста, приведённого ниже.\n"
            "Ты можешь переформулировать определения и объяснения естественным языком, "
            "чтобы студенту было понятно, но НЕ добавляй фактов, которых нет в контексте.\n"
            "Если контекст пуст или информации недостаточно — честно скажи, что не нашёл данных "
            "в базе знаний, НЕ придумывай ответ от себя.\n\n"
        )
        ...
        if any(
            ...
            prompt = (
                ...
                "Сравни эти понятия своими словами, опираясь СТРОГО на контекст.\n"
                "Если в контексте нет данных для сравнения — скажи об этом.\n"
            ...
            prompt = (
                base_instruction + f"Контекст из базы знаний ПиОИвИС:\n\n{context}\n\n"
                f"Вопрос студента: {question}\n\n"
                "Перечисли элементы своими словами, но ТОЛЬКО те, что есть в контексте.\n"
                "Если в контексте нет подходящих элементов — скажи об этом.\n"
            ...
            prompt = (
                base_instruction + f"Контекст из базы знаний ПиОИвИС:\n\n{context}\n\n"
                f"Вопрос студента: {question}\n\n"
                "Опиши связи термина своими словами, опираясь СТРОГО на контекст.\n"
                "Если в контексте нет связей — скажи об этом.\n"
            ...
            prompt = (
                ...
                "Если в контексте нет примеров — скажи об этом, НЕ придумывай свои.\n"
            ...
            prompt = (
                base_instruction + f"Контекст из базы знаний ПиОИвИС:\n\n{context}\n\n"
                f"Вопрос студента: {question}\n\n"
                "Ответь своими словами, опираясь СТРОГО на контекст.\n"
                "Если в контексте нет данных — скажи, что не нашёл информацию в базе знаний.\n"
